...
class Painter:
	...
	@staticmethod
	def _plot_cube(ax, x, y, z, dx, dy, dz, color='red', mode=2, linewidth=1, text="", fontsize=15, alpha=0.5):
	...
	@staticmethod
	def _plot_cylinder(ax, x, y, z, dx, dy, dz, color='red', text="", fontsize=10, alpha=0.2):
	...
	def plot_box_and_items(self, title="", alpha=0.2, write_num=False, fontsize=10):
		...
		for item in self.items:
			...
			color = item.color
			text = item.part_no if write_num else ""

			if item.typeof == 'cube':
				# plot item of cube
				self._plot_cube(
					ax_glob, float(x), float(y), float(z),
				...
				self._plot_cylinder(
					ax_glob, float(x), float(y), float(z),
					float(w), float(h), float(d),
					color=color, text=text, fontsize=fontsize, alpha=alpha
				)

			counter = counter + 1

		plt.title(title)
		self.set_axes_equal(ax_glob)
